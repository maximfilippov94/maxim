...
<?php
    ...
    $phone = setting('phone');
    $logo_raw = setting('logo_path') ?: 'assets/images/fanera63-logo.svg';
...
.megaLeftItem:hover,.megaLeftItem.active{background:rgba(191,127,43,.1);color:var(--copper2)}
...
.megaSub:hover{background:rgba(191,127,43,.08);color:var(--copper2)}
...
.mobileAccordion.open .mobileAccordionChevron{background:rgba(191,127,43,.12);border-color:rgba(191,127,43,.3);transform:rotate(180deg)}
...
.mobileCatBtn.open{color:var(--copper2);background:rgba(191,127,43,.1);border-color:rgba(191,127,43,.25)}
.mobileCatBtn svg{transition:transform .25s}
.mobileCatBtn.open svg{transform:rotate(180deg)}
/* Подкатегории */
.mobileCatSubs{max-height:0;overflow:hidden;transition:max-height .3s cubic-bezier(.4,0,.2,1);border-left:2px solid rgba(191,127,43,.25);margin-left:4px}
...
<header class="topbar">
  <a class="brand logoOnly" href="<?=h($logoHref)?>"><img src="<?=h($logo)?>" alt="Фанера63.рф"></a>
  <nav>
...
<div id="mobileMenu" class="mobileMenu" role="dialog" aria-modal="true" aria-label="Навигация">
  <div class="mobileMenuInner">
    <div class="mobileMenuTop">
      <div class="menuLogo"><img src="<?=h($logo)?>" alt="Фанера63.рф" class="menuLogoImg"></div>
...
function render_footer($pdo, $logoPath = null) {
    $items = get_nav_items($pdo);
    $logo_raw  = $logoPath ?: (setting('logo_path') ?: 'assets/images/fanera63-logo.svg');
    $logo = str_starts_with($logo_raw, '/') ? $logo_raw : '/' . $logo_raw;
    $phone = setting('phone');
    $ig_raw = setting('instagram');
    $tg_raw = setting('telegram');
...
<footer class="footer">
  <div class="footerBrand">
    <img src="<?=h($logo)?>" alt="Фанера63.рф" class="footerLogo">
    <p>Фанера и ОСБ с доставкой в Тольятти.</p>
